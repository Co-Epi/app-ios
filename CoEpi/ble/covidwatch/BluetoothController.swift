...
class BluetoothController: NSObject {
    ...
    private let delegate: BleAdapter
    ...
    func centralManager(
        ...
        if isNewlyDiscovered {
            ...
            if let advertisementDataServiceData = advertisementData[CBAdvertisementDataServiceDataKey] as? [CBUUID : Data],
                ...
                // Process incoming CEN (advertising data)
                delegate.didDiscoverCen(cen: uuidData)
        ...
        if let contactEventIdentifierCharacteristic = service.characteristics?.first(where: {
            ...
            if self.shouldReadContactEventIdentifier(from: peripheral) {
                ...
                if !self.writingConfigurationCharacteristics.contains(
                    ...
                    // Write my CEN in the peripheral ("inverse read")
                    let value = delegate.provideMyCen()
                    delegate.centralDidWrite(cen: value)
...
//                    let value = identifier.dataRepresentation
    ...
    func peripheral(
        ...
        do {
            ...
            // Process incoming CEN (characteristic read)
            delegate.didDiscoverCen(cen: value)
...
extension BluetoothController: CBPeripheralManagerDelegate {
    ...
    func peripheralManager(
        ...
        /****************************************************************/
        // CoEpi modification
        // Provide my CEN (characteristic read request)

        let value = delegate.provideMyCen()
        request.value = value
        delegate.peripheralWasRead(cen: value)
    ...
    func peripheralManager(
        ...
        for request in requests {
            do {
                ...
                /****************************************************************/
                // CoEpi modification
                // Process incoming CEN (characteristic write request)

                delegate.didDiscoverCen(cen: value)
                delegate.peripheralWasWrittenTo(cen: value)
