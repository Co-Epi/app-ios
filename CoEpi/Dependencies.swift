...
class Dependencies {
    ...
    private func registerAndInitNativeCore(container: DependencyContainer, fileSystem: FileSystem) {
        ...
        container.register(.singleton) { nativeCore as AlertsApi }
                                    ...
                                    alertFilterSettings: AlertFilterSettings)
    {
        container.register { HomeViewModel(
            startPermissions: try container.resolve(),
    ...
    private func registerRepos(container: DependencyContainer) {
        container.register(.singleton) { SymptomRepoImpl(
            inputManager: try container.resolve()
        ) as SymptomRepo }
        container.register(.singleton) { AlertRepoImpl(
    ...
    private func registerWiring(
        ...
        container.register(.eagerSingleton) { ScannedCensHandler(
            bleAdapter: try container.resolve(), tcnsRecorder: try container.resolve()
        )
        }
        container.register(.eagerSingleton) { PeriodicAlertsFetcher(
            alertRepo: try container.resolve()
        ) }
        container.register(.eagerSingleton) { RootNav() }

        container.register(.singleton) { SymptomRouterImpl(
            rootNav: try container.resolve()
        ) as SymptomRouter }
