...
final class AlamofireLogger: EventMonitor {
    func requestDidResume(_ request: Request) {
        let stringDecoding = String(decoding: $0, as: UTF8.self)
        let body = request.request.flatMap { $0.httpBody.map { stringDecoding } } ?? "None"
        let message = """
    ...
    func request<Value>(
        ...
        didParseResponse response: DataResponse<Value, Error>) {
...
func configureBackend(_ type: BackendType = .aws) -> BackendConfig {
    ...
    //curl -X GET "https://zmqh8rwdx4.execute-api.us-west-2.amazonaws.com/v4/tcnreport/0.4.0?intervalNumber=73581&intervalLength=21600"
        return BackendConfig(type: .awsUpdatedIntervalFormat,
...
class CoEpiNetworkingV4Tests: XCTestCase {
    ...
    let backend = configureBackend(.awsUpdatedIntervalFormat)
    ...
    func postTcnReportHelper(urlRequest: URLRequest) {
        ...
        do {
            _ = session.request(urlRequest).validate()
    ...
    func testReportInflation() {
        ...
        do {
            let signedReport = try SignedReport(
                serializedData: Data(base64Encoded: hexEncodedSignedReport)!)
            do {
    ...
    func testTcnClientCryptoNetworking() {
        do {
            // swiftlint:disable line_length
            let expectedRakBytes = "577cfdae21fee71579211ab02c418ee0948bacab613cf69d0a4a5ae5a1557dbb"
            let expectedRvkBytes = "fd8deb9d91a13e144ca5b0ce14e289532e040fe0bf922c6e3dadb1e4e2333c78"
            // swiftlint:enable line_length
            let expectedTckBytes = [
            ...
            let rak = ReportAuthorizationKey(
                reportAuthorizationPrivateKey: try .init(
                    rawRepresentation: expected_rak_bytes.hexDecodedData()))
            XCTAssertEqual(
                rak.reportAuthorizationPrivateKey.rawRepresentation.map {
                    String(format: "%02x", $0) }
                    .joined(), expected_rak_bytes)
            ...
            for index in 0..<(Int(startIndex)-1) {
                if index < 9 {
                    XCTAssertEqual(
                        ...
                        expected_tck_bytes[index])
                    XCTAssertEqual(
                        tck.temporaryContactNumber.bytes.map {
                            String(format: "%02x", $0) }.joined(),
                        expected_tcn_bytes[index])
                }
                tck = tck.ratchet()!
            }
...
//        let intervalLength : Int64 = 86400
        ...
        for var i: Int64 in 0...3 {
            ...
            //curl -X GET "https://zmqh8rwdx4.execute-api.us-west-2.amazonaws.com/v4/tcnreport/0.4.0?intervalNumber=73581&intervalLength=21600"
            case .golang, .awsUpdatedIntervalFormat:
    ...
    private func executeGet(url: String) {
        ...
        let session = Session(configuration: configuration, eventMonitors: [ AlamofireLogger()])
        _ = session.request(url).validate()
            .cURLDescription { description in
                NSLog(description)
            }
